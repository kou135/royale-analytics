...
2. **`ra fetch`**: プロフィールと battlelog を取得し、重複排除して SQLite に追記する。出力の「新規取得数」と「ギャップ警告（取りこぼし疑い）」を確認する。
...
7. **無課金現実的・進行連動。** 助言は実カードレベルと level_deficits（カードレベル差）を踏まえ、実行可能な手順に落とす。
...
## 次アクション（無課金現実的・進行連動）
- level_deficits（実測）を踏まえ、Musketeer のレベル上げを優先（実行可能な手順）。
```
